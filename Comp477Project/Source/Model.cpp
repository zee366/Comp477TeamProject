...
bool Model::ParseLine(const std::vector<ci_string> &token)
{
    if (token.empty() == false)
	{
		...
		else if (token[0] == "scaling")
		{
			...
			SphereRadius = static_cast<float>(atof(token[2].c_str()));;
		}
...
glm::mat4 Model::GetWorldMatrix() const
{
	...
	// Build the world matrix from the position, scaling and rotation informations
	...
	else
	{
		...
		float angle = glm::dot(vec3(0.0f, 0.0f, -1.0f), normalize(mVelocity));
		quat direction = glm::angleAxis(acos(angle), glm::cross(vec3(0.0f, 0.0f, -1.0f), normalize(mVelocity)));
		r = glm::mat4_cast(direction);
	}
	worldMatrix = tr * r * s;
