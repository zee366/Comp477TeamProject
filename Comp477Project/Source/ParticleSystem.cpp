...
: mpDescriptor(descriptor), mpEmitter(emitter), timeSinceLastParticleEmitted(0.0f)
{
	...
	// set radius of bounding sphere that surrounds the particle system
	mBoundingRadius = (mpDescriptor->velocity).length() * (mpDescriptor->totalLifetime + mpDescriptor->totalLifetimeRandomness);
}
...
ParticleSystem::~ParticleSystem()
{
	...
	{
		World::GetInstance()->RemoveBillboard(&(*it)->billboard, this->mpDescriptor->GetName());
...
void ParticleSystem::Update(float dt)
{
    ...
    if (timeSinceLastParticleEmitted > averageTimeBetweenEmission && mInactiveParticles.size() > 0)
    {
		...
		World::GetInstance()->AddBillboard(&newParticle->billboard, this->mpDescriptor->GetName());
        ...
        // Initial Random Particle Velocity vector
        //
	...
	if (mParticleList.size() > 0) {
		for (std::list<Particle*>::iterator it = mParticleList.begin(); it != mParticleList.end(); )
		{
			...
			// Update each particle parameters
			//
			...
			if (p->currentTime > p->lifeTime)
			{
				...
				World::GetInstance()->RemoveBillboard(&(p->billboard), this->mpDescriptor->GetName());
...
{
	return mpEmitter->GetPosition();
}

bool ParticleSystem::IsHidingPrey(glm::vec3& origin, glm::vec3& target)
{
	float targetDistance = glm::distance(target, origin);

	if (mParticleList.size() > 0) {
		for (std::list<Particle*>::iterator it = mParticleList.begin(); it != mParticleList.end(); ++it)
		{
			float billboardDistance = glm::distance((*it)->billboard.position, origin);
			float theta = glm::acos(dot(billboardDistance, targetDistance));
			float distance = billboardDistance * tan(theta);
			
			// check that the billboard is closer and the billboard covers the prey
			if (billboardDistance < targetDistance && distance < ((*it)->billboard.size.x / 2))
				return true;
		}
	}
	return false;
...
bool ParticleSystem::IntersectsRay(glm::vec3 source, glm::vec3 direction) {
	...
	if (pow(origin.x + t * normDirection.x - center.x, 2) + pow(origin.y + t * normDirection.y - center.y, 2)
		+ pow(origin.z + t * normDirection.z - center.z, 2) <= pow(getBoundingRadius(), 2)) {
		return true;
	}
	else {
		return false;
