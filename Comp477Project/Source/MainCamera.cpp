...
MainCamera::MainCamera(glm::vec3 position) : Camera(), mPosition(position), mLookAt(0.0f, 0.0f, -1.0f), mHorizontalAngle(90.0f), mVerticalAngle(0.0f), mSpeed(5.0f), mAngularSpeed(2.5f)
{
}
...
void MainCamera::toggleMouse(bool value) {
	freeWill = value;
}
void MainCamera::setPosition(vec3 position) {
	mPosition = position;
}

void MainCamera::setLookAt(vec3 newLookAt) {
	mLookAt = newLookAt;
}